...
const Navbar = ({setOpen}) => {
  ...
  return (
    <div  className='w-full max-w-screen-4xl py-10 fixed -top-1 lg:shadow-none ' style={{backgroundColor: "#E9F7EE", zIndex: "50" }} >
    <header className=' flex lg:flex-row flex-col justify-between lg:items-center gap-10 w-11/12 mx-auto sha' style={{maxWidth: "1440px"}}>
        ...
        <button onClick={() => setShow(!show)}>{show ? <IoCloseOutline className='text-4xl lg:hidden'/> :<TiThMenu className='text-4xl lg:hidden'/>}</button>
        </div>

        <nav className={show ? "block" :"lg:block hidden"} onClick={() => setShow(false)}>
            <ul className='pt-3 flex lg:flex-row flex-col gap-10'>
            <li><Link offset={-50} className="text-dark-emphasis text-[20px] text-black" to="home"> Home</Link></li>
            <li><Link className="text-dark-emphasis text-[20px] text-black" to="about"> About</Link></li>
            <li><Link className="text-dark-emphasis text-[20px] text-black" to="speaker"> Speaker</Link></li>
            <li><Link  offset={-50} className="text-dark-emphasis text-[20px] text-black" to="video"> Video</Link></li>
